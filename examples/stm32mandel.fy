...
class Signed
  ...
  def -@
    invokeprimitive int_sub(0s64, self)
  end
...
def main
  ...
  RCC.CFGR = RCC.CFGR.set_pllmul(0b0111u32).set_pllsrc(true)
  ...
  lcd.clip(0x02u8, 0x02u8, 0x81u8, 0x81u8)

  let mut cx = -0x24ccc_s64
  let mut cy = -0x14ccc_s64
  let mut dx = 0x11eb_s64
  let mut dy = 0x0ccc_s64

  let mandelconverger = (real, imag, iters, creal, cimag) do
    let real_sq = (real * real) >> 16s64
    let imag_sq = (imag * imag) >> 16s64
    if iters > 100u8 or real_sq + imag_sq > 0x40000s64
      iters
    else
      mandelconverger.call(real_sq - imag_sq + creal,
                           2s64 * ((real * imag) >> 16s64) + cimag,
  ...
  while true
    ...
    128u32.times((_) do
      ...
      128u32.times((_) do
        ...
        lcd.put_pixel(density * 20u8, density * 20u8, density * 20u8)
    ...
    while not pressed
      if not GPIOC.get(0_u32)    # left
        cx -= 20s64 * dx; pressed = true
      elsif not GPIOC.get(1_u32) # top
        cy -= 20s64 * dx; pressed = true
      elsif not GPIOC.get(2_u32) # right
        cx += 20s64 * dy; pressed = true
      elsif not GPIOC.get(4_u32) # bottom
        cy += 20s64 * dy; pressed = true
      elsif not GPIOC.get(3_u32) # center
        let dx_old = dx
        let dy_old = dy
        dx = (dx * 0xC000s64) >> 16s64
        dy = (dy * 0xC000s64) >> 16s64
        cx += (dx_old - dx) * 64s64
        cy += (dy_old - dy) * 64s64
        pressed = true
      end
